...
def read_data(file_name):
    print (get_num_columns(file_name))
    ...
    df.columns = [eu.to_excel(index)for index in df.columns]
...
def clean_df_cell(cell):
    cell = cell.strip()
    cell = cell.upper() if cell != ERROR_VALUE else cell
    return cell
...
def isvalid_alphanumeric(key, dataset):
    ...
    tuple = get_index_by_alphanumeric(key, True)
...
def get_cell_by_alphanumeric(key, dataset):
    index = get_index_by_alphanumeric(key)
...
def get_index_by_alphanumeric(key, add_cache=True):
    cache = INDEX_MAP.get(key)

    if cache:
        return cache
    # Parse cell value to column and row, eg B7 - row 7, column 2
    column = eu.from_excel(''.join(filter(str.isalpha, key))) - 1
...
# Node code recipe is from https://stackoverflow.com/questions/48983939/convert-a-number-to-excel-s-base-26
def divmod_excel(n):
    a, b = divmod(n, 26)
    if b == 0:
        return a - 1, b + 26
